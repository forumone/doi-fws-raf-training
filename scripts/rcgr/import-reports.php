...
// Include the name import functions.
require_once __DIR__ . '/import-names.php';
...
// Track imported names by permit.
$imported_names = [];
...
function process_report_row(
  ...
  try {
    ...
    // If this is a current report (not a historical revision), import names for this permit.
    if (!$is_revision) {
      $names_result = import_names_for_permit($data['permit_no'], $data['report_year'], $_rcgr_import_logger);
      $_rcgr_import_logger->notice(sprintf(
        'Names import result for permit %s: Created: %d, Updated: %d, Skipped: %d',
        $data['permit_no'],
        $names_result['created'],
        $names_result['updated'],
        $names_result['skipped']
      ));
    }
